  ...
  int cmd;
{
  ...
  for (;;)
  {
	...
	{
      /* 040423.Lcaool:修改在看板文章最後一篇按 KEY_DOWN 或 KEY_PGDN 會翻到最前 */
	  /*
	  if (zone == XZ_POST)
    ...
    /* 040520.Lcaool:修改，仿[無名] e 接文，故將原本 e 退出文章列表的按鍵拔掉 */
    ...
    else if (cmd == ' ' || cmd == KEY_PGDN /* 041030.Lacool:修改文章置底按鍵 || cmd == 'N' */)
    {
